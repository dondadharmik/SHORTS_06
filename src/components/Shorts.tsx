...
const Shorts: React.FC = () => {
  ...
  const [currentSlide, setCurrentSlide] = useState(0);
  const [isMobile, setIsMobile] = useState(false);
  const [videoClicked, setVideoClicked] = useState(false); // State to track if a video is clicked
  const touchStartX = useRef(0);
  ...
  useEffect(() => {
    const handleTouchMove = (e: TouchEvent) => {
      if (videoClicked) {
        e.preventDefault(); // Prevent horizontal scrolling if a video is clicked
      }
    };
  ...
  const handleTouchMove = (e: React.TouchEvent<HTMLDivElement>) => {
    const touchEndX = e.touches[0].clientX;
    const difference = touchStartX.current - touchEndX;
    if (Math.abs(difference) > 50 && !videoClicked) {
      if (difference > 0) {
        handleNextClick();
      } else {
        handlePrevClick();
  ...
  const handleVideoClick = (videoIndex: number) => {
    setVideoClicked(true); // Set videoClicked to true when a video is clicked
    router.push(`/video/${videoIndex}`);
  };

  const handleVideoLoad = () => {
    setVideoClicked(false); // Set videoClicked to false when the video finishes loading
  };
